...
test_that("preview_metadata_files processes metadata correctly", {
  ...
  expect_true(grepl("E \\= mc\\^2", result_df$equations[1]))
  expect_true(grepl("F \\= ma", result_df$equations[1]))
  expect_true(grepl("P \\= IV", result_df$equations[2]))
  expect_true(grepl("N/A", result_df$equations[3]))

  expect_true(grepl("Test note 1", result_df$notes[1]))
  expect_true(grepl("Test note 2", result_df$notes[1]))
  expect_true(grepl("Single note", result_df$notes[2]))
  expect_true(grepl("N/A", result_df$notes[3]))

  expect_true(grepl("abbr1", result_df$abbreviations[1]))
  expect_true(grepl("abbr2", result_df$abbreviations[1]))
  expect_true(grepl("abbr_single", result_df$abbreviations[2]))
  expect_true(grepl("N/A", result_df$abbreviations[3]))
