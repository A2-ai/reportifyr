test_that("update_object_footnotes throws an error if file does not exist", {
  ...
  expect_error(update_object_footnotes(temp_json), regexp = "The metadata associated with the specified file does not exist")
})
...
test_that("update_object_footnotes appends footnotes when overwrite is FALSE", {
  ...
  update_object_footnotes(temp_json, meta_equations = "New equation", meta_notes = "New note", meta_abbrevs = "abbr2", overwrite = FALSE)

  updated_metadata <- jsonlite::fromJSON(temp_json, simplifyVector = TRUE)

  expect_true("New equation" %in% updated_metadata$object_meta$footnotes$equations)
...
test_that("update_object_footnotes overwrites footnotes when overwrite is TRUE", {
  ...
  update_object_footnotes(temp_json, meta_equations = "Overwritten equation", meta_notes = "Overwritten note", meta_abbrevs = "abbr_overwrite", overwrite = TRUE)

  updated_metadata <- jsonlite::fromJSON(temp_json, simplifyVector = TRUE)

  expect_equal(updated_metadata$object_meta$footnotes$equations, "Overwritten equation")
  expect_equal(updated_metadata$object_meta$footnotes$notes, "Overwritten note")
  expect_equal(updated_metadata$object_meta$footnotes$abbreviations, "abbr_overwrite")
...
test_that("update_object_footnotes adjusts file path if not .json", {
  ...
  json_file_path <- paste0(tools::file_path_sans_ext(temp_txt), "_txt_metadata.json")
  jsonlite::write_json(metadata, json_file_path)

  update_object_footnotes(temp_txt, meta_equations = "New equation", meta_notes = "New note", meta_abbrevs = "abbr2")
